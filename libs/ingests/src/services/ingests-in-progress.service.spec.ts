...
import { IngestsInProgressService } from './ingests-in-progress.service';

describe('Service: IngestsInProgressService', () => {
  let service: IngestsInProgressService;
  ...
  beforeEach(() => {
    ...
    service = TestBed.get(IngestsInProgressService);
  });

    beforeEach(inject([IngestsInProgressService], s => {
        service = s;
      }));
  ...
  it('should return array of IngestsInProgress objects for get request', fakeAsync(() => {
      let response = [
           {
      ...
      service.getIngestsInProgress(3).subscribe(res => {
